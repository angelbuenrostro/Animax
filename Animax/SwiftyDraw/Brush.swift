...
public class Brush {
    ...
    /// Sets an optional brush pattern
    public var pattern: CGPattern?
    
    public init(color: UIColor = .black, width: CGFloat = 3, opacity: CGFloat = 1, adjustedWidthFactor: CGFloat = 1, blendMode: CGBlendMode = .normal, pattern: CGPattern? = nil) {
        self.color = color
